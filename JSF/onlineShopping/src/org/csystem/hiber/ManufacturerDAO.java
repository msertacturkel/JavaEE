 ...
 * Manufacturer entities. Transaction control of the save(), update() and
 * delete() operations can directly support Spring container-managed
 * transactions or they can be augmented to handle user-managed Spring
 * transactions. Each of these methods provides additional information for how
 * to configure it for the desired type of transaction control.
 * 
 * @see org.csystem.hiber.Manufacturer
 * @author MyEclipse Persistence Tools
 */

public class ManufacturerDAO extends BaseHibernateDAO {
	private static final Logger log = LoggerFactory
			.getLogger(ManufacturerDAO.class);
	// property constants
	...
	public void save(Manufacturer transientInstance) {
		log.debug("saving Manufacturer instance");
		try {
	...
	public Manufacturer findById(java.lang.Integer id) {
		...
		try {
			Manufacturer instance = (Manufacturer) getSession().get(
					"org.csystem.hiber.Manufacturer", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Manufacturer instance) {
		log.debug("finding Manufacturer instance by example");
		try {
			List results = getSession()
					.createCriteria("org.csystem.hiber.Manufacturer")
	...
	public List findByRep(Object rep) {
		return findByProperty(REP, rep);
	}
	...
	public Manufacturer merge(Manufacturer detachedInstance) {
		...
		try {
			Manufacturer result = (Manufacturer) getSession().merge(
					detachedInstance);
