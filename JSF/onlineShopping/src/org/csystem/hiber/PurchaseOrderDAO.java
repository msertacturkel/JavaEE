 ...
 * 
 * @see org.csystem.hiber.PurchaseOrder
...
public class PurchaseOrderDAO extends BaseHibernateDAO {
	...
	public PurchaseOrder findById(java.lang.Integer id) {
		...
		try {
			PurchaseOrder instance = (PurchaseOrder) getSession().get(
					"org.csystem.hiber.PurchaseOrder", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(PurchaseOrder instance) {
		...
		try {
			List results = getSession()
					.createCriteria("org.csystem.hiber.PurchaseOrder")
	...
	public List findByShippingCost(Object shippingCost) {
		return findByProperty(SHIPPING_COST, shippingCost);
	}

	public List findByFreightCompany(Object freightCompany) {
		return findByProperty(FREIGHT_COMPANY, freightCompany);
	}
